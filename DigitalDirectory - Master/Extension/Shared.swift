...
enum ActionDescriptor {
  ...
  func image(forStyle style: ButtonStyle, displayMode: ButtonDisplayMode, height: CGFloat = 100) -> UIImage? {
    ...
    if #available(iOS 13.0, *) {
      ...
      if style == .backgroundColor {
        ...
        return circularIcon(with: UIColor(hexString: "EC3D2C").withAlphaComponent(0.8), size: CGSize(width: 70, height: height), icon: image)
      }
    } else {
  ...
  func color(forStyle style: ButtonStyle) -> UIColor {
    ...
    case .trash: return #colorLiteral(red: 1, green: 0.2117647059, blue: 0.3176470588, alpha: 1)
    }
    #endif
  }
